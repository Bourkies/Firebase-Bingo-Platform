...
function parseEvidenceLog(logString) {
    ...
    // 1. Try to parse as Summary String (generated by indexController diff logic)
    ...
    try {
        ...
        rawChanges.forEach(change => {
            // Parse "Modified: 'OldName' to 'NewName' for link (Link)"
            const modMatch = change.match(/^Modified: '(.*?)' to '(.*?)' for link \((.*)\)$/);
            if (modMatch) {
                parsedItems.push({ type: 'modified_name', oldName: modMatch[1], newName: modMatch[2], link: modMatch[3] });
                return;
            }

            // Parse "Added: Name (Link)"
            const addMatch = change.match(/^Added: (.*?) \((.*)\)$/);
            if (addMatch) {
                added.push({ name: addMatch[1], link: addMatch[2] });
                return;
            }

            // Parse "Removed: Name (Link)"
            const remMatch = change.match(/^Removed: (.*?) \((.*)\)$/);
            if (remMatch) {
                removed.push({ name: remMatch[1], link: remMatch[2] });
                return;
            }
        ...
        // Heuristic: Check for Link Changes (Same Name, Removed Old Link, Added New Link)
        const processedAdded = new Set();
        
        removed.forEach(rem => {
            // Find an added item with the same name that hasn't been processed.
            // We ignore 'No Name' to avoid falsely merging unrelated unnamed links.
            const matchIndex = added.findIndex((add, idx) => !processedAdded.has(idx) && add.name === rem.name && add.name !== 'No Name');
            
            if (matchIndex !== -1) {
                const add = added[matchIndex];
                processedAdded.add(matchIndex);
                parsedItems.push({ type: 'modified_link', name: rem.name, oldLink: rem.link, newLink: add.link });
            } else {
                parsedItems.push({ type: 'removed', name: rem.name, link: rem.link });
            }
        });

        // Add remaining added items
        added.forEach((add, idx) => {
            if (!processedAdded.has(idx)) parsedItems.push({ type: 'added', name: add.name, link: add.link });
        });

        // Convert to HTML
        return parsedItems.map(item => {
            if (item.type === 'modified_name') return `<strong>Modified Name:</strong> <span class="old-val">"${item.oldName}"</span> ➜ <span class="new-val">"${item.newName}"</span> <span class="meta-info">(${item.link})</span>`;
            if (item.type === 'modified_link') return `<strong>Modified Link:</strong> <span class="old-val">"${item.oldLink}"</span> ➜ <span class="new-val">"${item.newLink}"</span> <span class="meta-info">(${item.name})</span>`;
            if (item.type === 'added') return `<strong>Added:</strong> <span class="new-val">"${item.name}"</span> <span class="meta-info">(${item.link})</span>`;
            if (item.type === 'removed') return `<strong>Removed:</strong> <span class="old-val">"${item.name}"</span> <span class="meta-info">(${item.link})</span>`;
            return item.text;
        });
        }
    } catch (e) {
    ...
    // 2. Try to parse as JSON Array (Raw state logging, e.g. from Create Submission)
    // Format: '[{"link":"...","name":"..."}]'
    try {
    ...
    // 3. Try to parse as Semicolon separated string (Legacy/Simple Raw state)
    // Format: 'url1; url2'
